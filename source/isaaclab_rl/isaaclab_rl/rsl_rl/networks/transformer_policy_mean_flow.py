...
class TransformerPolicyMeanFlow(PreTrainedModel):
    def __init__(self, 
        ...
        # attention masks
        total_tokens = self.proprio_horizon + self.num_action_tokens + self.control_obs_horizon
        ...
        self.attn_mask[self.proprio_horizon:self.proprio_horizon + self.num_action_tokens, -self.control_obs_horizon:] = False
    ...
    def forward(self, proprio: torch.Tensor, control: torch.Tensor, r: torch.Tensor, t: torch.Tensor,
        ...
        actions = self.action_output_proj(input[:, 
                                                self.proprio_horizon:self.proprio_horizon + self.num_action_tokens
                                            ].flatten(start_dim=1))
        control_obs_velocity = self.control_obs_output_proj(input[:, self.proprio_horizon + self.num_action_tokens:])
