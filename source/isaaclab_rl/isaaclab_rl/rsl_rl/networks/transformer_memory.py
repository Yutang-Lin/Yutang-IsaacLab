...
class TransformerMemory(torch.nn.Module):
    def __init__(self, input_size, num_layers=1, hidden_size=64,
                 hidden_token_num: int = 4):
        super().__init__()
        # Transformer
        self.projector = nn.Linear(input_size, hidden_size)
        self.transformer = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(hidden_size, nhead=hidden_size // 16,
                                       dim_feedforward=hidden_size * 2,
                                       dropout=0.05,
                                       activation="gelu"),
            num_layers=num_layers,
        )
        self.initial_memory_tokens = nn.Parameter(torch.randn(hidden_token_num, hidden_size))
