...
class LNNStyleTransformerLL(nn.Module):
    def __init__(self, 
                 d_model,
                 proprio_dim,
                 text_dim,
        ...
        num_tokens = num_history_tokens + num_task_tokens + num_latent_tokens + num_input_tokens
        self.register_buffer('attn_mask', torch.ones(1, num_tokens, num_tokens, dtype=torch.bool))
        self.attn_mask: torch.Tensor # type hint
        # mask latent tokens to let it only attend to task tokens
        self.attn_mask[:, :num_history_tokens, num_history_tokens + num_task_tokens:-num_input_tokens] = False
        self.attn_mask[:, -num_input_tokens:, num_history_tokens + num_task_tokens:-num_input_tokens] = False
        self.attn_mask[:, num_history_tokens + num_task_tokens:-num_input_tokens, :num_history_tokens] = False
        self.attn_mask[:, num_history_tokens + num_task_tokens:-num_input_tokens, -num_input_tokens:] = False

        self._save_dict = dict()

    def encode_inputs(self, proprio, text=None, latent=None):
        proprio = self.proprio_proj(proprio)
        if text is not None:
            text = self.text_proj(text)
        if latent is not None:
            tokenized_latent = self.latent_tokenizer(latent)
        return proprio, text, tokenized_latent, latent
    ...
    def forward(self, input: TensorDict, hidden_states: torch.Tensor | None,
        ...
        proprio, text, tokenized_latent, latent_ori = self.encode_inputs(**transpose_input)
        ...
        tokenized_latent = tokenized_latent.view(batch_size, seq_length, self.num_latent_tokens, self.d_model)
        if not no_encode_latent:
            latent_mu, latent_logvar = self.latent_encoder(tokenized_latent).chunk(2, dim=-1)
            latent_std = torch.exp(0.5 * latent_logvar)
            latent = torch.randn_like(latent_mu) * latent_std + latent_mu
        else:
            latent = tokenized_latent
        ...
        task_tokens = text + self.task_tokens_embeddings.unsqueeze(0).unsqueeze(0)
        latent_tokens = latent + self.latent_tokens_embeddings.unsqueeze(0).unsqueeze(0)
        attn_mask = self.attn_mask.repeat(batch_size, 1, 1)
        ...
        for s in range(seq_length):
            ...
            if compute_latent_loss:
                if masks is not None:
                    ...
                    step_mask = 1.0
                recons_latent = last_output[:, self.num_history_tokens + self.num_task_tokens:-self.num_proprio_tokens]
                ...
                recons_loss = F.mse_loss(recons_latent.view(batch_size, self.latent_dim) * step_mask, 
                                         latent_ori[:, s] * step_mask, reduction='none').mean(dim=-1).sum()
...
class TransformerMemoryLL(Memory):
    def __init__(self, proprio_dim, 
        ...
        self.rnn = LNNStyleTransformerLL(d_model, 
                                       proprio_dim, 
                                       text_dim,
                                       motion_dim,
                                       num_input_tokens,
