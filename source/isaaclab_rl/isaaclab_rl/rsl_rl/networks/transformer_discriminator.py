...
class TransformerDiscriminator(nn.Module):
    def __init__(self, 
        ...
        self.input_embeddings = nn.Parameter(torch.randn(num_input_tokens, d_model))
    ...
    def forward(self, input: torch.Tensor):
        input = self.input_proj(input).view(input.shape[0], self.num_input_tokens, self.d_model)
        input = torch.cat([input + self.input_embeddings.unsqueeze(0), 
                           self.cls_token.repeat(input.shape[0], 1, 1)], dim=1)
