...
class HybridStorage:
    ...
    def __init__(
        ...
        if training_type == "rl":
            ...
            self.fwd_returns = torch.zeros(num_transitions_per_env, num_envs, 1, device=self.device)
            self.rev_returns = torch.zeros(num_transitions_per_env, num_envs, 1, device=self.device)

            self.advantages = torch.zeros(num_transitions_per_env, num_envs, 1, device=self.device)
            self.history_length = torch.zeros(num_transitions_per_env, num_envs, 1, device=self.device, dtype=torch.long)
            self.history_returns = torch.zeros(num_envs, 1, device=self.device)
    ...
    def clear(self, clear_history: bool = False):
        self.step = 0
        if clear_history:
            self.history_returns[:] = 0.0

    def compute_returns(self, gamma_f, gamma_r, alpha, normalize_advantage: bool = True):
        for step in reversed(range(self.num_transitions_per_env)):
            # if we are at the last step, bootstrap the return value
            rev_step = self.num_transitions_per_env - 1 - step

            if step == self.num_transitions_per_env - 1:
                next_values = 0.0
            else:
                next_values = self.fwd_returns[step + 1]
            ...
            self.fwd_returns[step] = self.rewards[step] + next_is_not_terminal * gamma_f * next_values

            if rev_step == 0:
                last_values = self.history_returns
            else:
                last_values = self.rev_returns[rev_step - 1]
            self.rev_returns[rev_step] = self.rewards[rev_step] + next_is_not_terminal * gamma_r * last_values

            if rev_step == 0:
                self.history_length[rev_step] = (self.history_length[step] + 1) * next_is_not_terminal
            else:
                self.history_length[rev_step] = (self.history_length[rev_step - 1] + 1) * next_is_not_terminal
            # self.rev_returns[rev_step] = self.rev_returns[rev_step] / ( (1 - torch.pow(gamma_r, self.history_length[rev_step] + 1)) / (1 - gamma_r) )

        self.history_returns[:] = self.rev_returns[self.num_transitions_per_env - 1]
        self.rev_returns[:] = (self.rev_returns - self.rev_returns.mean(dim=1, keepdim=True)) / (self.rev_returns.std(dim=1, keepdim=True) + 1e-8)
        self.fwd_returns[:] = (self.fwd_returns - self.fwd_returns.mean(dim=1, keepdim=True))

        prod_alpha = alpha
        for step in range(self.num_transitions_per_env):
            self.returns[step] = prod_alpha * self.fwd_returns[step] + (1.0 - prod_alpha) * self.rev_returns[step]
            prod_alpha = prod_alpha * alpha

        # Compute the advantages
        # GRPO's advantage
        self.advantages = self.returns * (self.dones | (self.history_length.float() > self.history_length.float().mean(dim=1, keepdim=True)))
