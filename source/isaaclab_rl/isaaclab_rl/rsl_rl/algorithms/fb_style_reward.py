...
class FBStyleReward:
    def __init__(self, input_dim: int,
        ...
        self.reward_clip_eps = reward_clip_eps
        ...
        self.b_network = nn.Sequential(*layers)
        self.b_network.to(device)

        layers = []
        layers.append(nn.Linear(input_dim + latent_dim, hidden_dims[0]))
        layers.append(deepcopy(activation))
        for i in range(len(hidden_dims) - 1):
            layers.append(nn.Linear(hidden_dims[i], hidden_dims[i + 1]))
            if layer_norm:
        ...
        self.d_network = nn.Sequential(*layers)
        self.d_network.to(device)
        ...
        if training:
            ...
            self.b_optimizer = torch.optim.AdamW(self.b_network.parameters(), lr=learning_rate, weight_decay=weight_decay)
            self.d_optimizer = torch.optim.AdamW(self.d_network.parameters(), lr=learning_rate, weight_decay=weight_decay)
    ...
    def _compute_gradient_penalty_(self, interpolated):
        ...
        d_interpolated = self.d_network(interpolated)
    ...
    def _optimize_amp(self, gen_batch, ref_batch) -> tuple[float, float]:
        score_ref = self.d_network(ref_batch)
        score_gen = self.d_network(gen_batch)
        ...
        norm = nn.utils.clip_grad_norm_(self.d_network.parameters(), self.max_grad_norm)
    ...
    def compute_reward(self, obs: torch.Tensor, scale: float | torch.Tensor = 1.0):
        ...
        amp_score = self.d_network(features).view(self.num_envs)
    ...
    def reduce_parameters(self):
        ...
        grads = [param.grad.view(-1) for param in self.d_network.parameters() if param.grad is not None]
        grads.extend([param.grad.view(-1) for param in self.b_network.parameters() if param.grad is not None])
        ...
        # Get all parameters
        all_params = [self.d_network.parameters(), self.b_network.parameters()]

        # Update the gradients for all parameters with the reduced gradients
        offset = 0
        for network in all_params:
