...
from isaaclab_rl.rsl_rl.modules import ActorCriticDP

from itertools import chain
from rsl_rl.modules.rnd import RandomNetworkDistillation
from rsl_rl.utils import string_to_callable
from isaaclab_rl.rsl_rl.storage import DPStorage

class RDPPO(RslRlPPO):
    policy: ActorCriticDP
    ...
    def __init__(self, policy,
        ...
        *args, **kwargs):
        ...
        self.storage: DPStorage = None  # type: ignore
        self.transition = DPStorage.Transition()
    ...
    def act(self, obs, critic_obs, infos=None, **kwargs):
        if self.policy.is_recurrent:
            self.transition.hidden_states = self.policy.get_hidden_states()
        # compute the actions and values
        self.transition.initial_noise = self.policy.generate_noise(obs.shape[0], self.device)
        if infos is not None:
            assert 'reference_actions' in infos
            self.transition.reference_actions = infos['reference_actions']
        else:
            self.transition.reference_actions = self.transition.initial_noise * 0.0
            print('[WARNING] No reference actions provided. Using zero noise as reference.')
        self.transition.actions = self.policy.act(obs, self.transition.initial_noise).detach() # type: ignore
    ...
    def update(self):  # noqa: C901
        ...
        for update_id, (
            ...
            # -- actor
            self.policy.act(actor_obs_batch, initial_noise_batch, masks=masks_batch, hidden_states=hid_states_batch[0])
            ...
            if hasattr(self.policy, "extra_loss"):
                extra_loss = self.policy.extra_loss(
                    ...
                    reference_actions_batch=reference_actions_batch,
                    initial_noise_batch=initial_noise_batch,
                )
